...
public class TestFarmToMarketController {
    ...
    public void testBookEquipment() throws Exception {
        ...
        ResultActions response = mockMvc.perform(put("/farmmarket/book/{equipId}/{userid}/{quantitycount}", e.getEquipmentId(),e.getUser().getId(),ecount)
    ...
    public void testUpdateEquipmentDetails() throws Exception {
        ...
        ResultActions response = mockMvc.perform(put("/farmmarket/equipmentDetails/{uid}", equipment.getEquipmentId(),user1.getId())
                .contentType(MediaType.APPLICATION_JSON)
